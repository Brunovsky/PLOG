...
There are two more data compounds: \vt{val/4}, which holds a board evaluation (see \autoref{subsec:boardscoring}) and \vt{node/6}, which is a node in the bot's lookahead tree analysis (see \autoref{subsec:bot}).

Several list data structures are abstracted away either by library  or auxiliary predicates which act on them: \vt{Move} is merely a row column pair [$R$,$C$]; \vt{Range} is a list [$I$,$J$] representing an interval (which includes $I$ and $J$); \vt{Cap} is the pair of captures [Wc,Bc]; the abstractions are usually hidden behind variables \vt{Game}, \vt{Val} and \vt{Node}, etc.

\subsection{Options}
\label{subsec:options}

The options accepted in the \vt{Options} list are the following (use \vt{help}, \vt{help(Option)} or \vt{help_menu} to get a description on the console):
...
\begin{itemize}
	...
	\item \vtb{padding(P)}, \vt{P} in $\{0,1,2,3,...\}$\\
	Padding of the active subboard used by the search tree.
	\item \vtb{width([W0,W1,...])}, \vt{Wi} in $\{1,2,3,...\}$\\
